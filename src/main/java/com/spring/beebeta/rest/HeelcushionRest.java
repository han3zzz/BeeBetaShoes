...
public class HeelcushionRest {
    ...
    @PostMapping()
    public ResponseEntity<?> add(@Valid @RequestBody HeelcushionRequest request, BindingResult result){
        if (result.hasErrors()){
    ...
    public ResponseEntity<?> update(@PathVariable("id") Integer Id,@Valid @RequestBody HeelcushionRequest request, BindingResult result){
        if (result.hasErrors()){
            List<ObjectError> list = result.getAllErrors();
            return ResponseEntity.badRequest().body(list);
        }
