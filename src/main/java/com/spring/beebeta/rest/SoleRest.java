...
import com.spring.beebeta.service.SoleService;
...
public class SoleRest {
    ...
    @PostMapping()
    public ResponseEntity<?> add(@Valid @RequestBody SoleRequest request, BindingResult result){
        if (result.hasErrors()){
    ...
    public ResponseEntity<?> update(@PathVariable("id") Integer Id,@Valid @RequestBody SoleRequest request, BindingResult result){
        if (result.hasErrors()){
            List<ObjectError> list = result.getAllErrors();
            return ResponseEntity.badRequest().body(list);
        }
